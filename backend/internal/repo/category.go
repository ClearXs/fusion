...
func (a *CategoryRepository) Save(insert *domain.Category, opts ...*options.InsertOneOptions) (uint64, error) {
	coll := a.Db.Collection(CategoryCollection)
	return handleSave[domain.Category](coll, insert, opts...)
}

func (a *CategoryRepository) SaveMany(inserts []interface{}, opts ...*options.InsertManyOptions) ([]uint64, error) {
	coll := a.Db.Collection(CategoryCollection)
