...
func (a *VisitRepository) Save(insert *domain.Visit, opts ...*options.InsertOneOptions) (uint64, error) {
	coll := a.Db.Collection(VisitCollection)
	return handleSave[domain.Visit](coll, insert, opts...)
}

func (a *VisitRepository) SaveMany(inserts []interface{}, opts ...*options.InsertManyOptions) ([]uint64, error) {
	coll := a.Db.Collection(VisitCollection)
